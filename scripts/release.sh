...
if [[ "$1" == "release" ]]; then
  ...
  if [[ -z $(git branch --list "origin/main") ]]; then
    echo "branch 'origin/main' does not exist"
    exit
  fi

  if [[ -z $(git branch --list "origin/releases/$2") ]]; then
  ...
  git merge --ff-only "origin/releases/$2" -m "Merge branch 'releases/$2'"
  echo ""
  ...
  git branch -d "releases/$2" &> /dev/null
